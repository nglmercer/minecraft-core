...
export class MinecraftServerManager {
  ...
  async downloadServer(options: DownloadOptions): Promise<DownloadResult> {
    ...
    if (options.buildId) {
      ...
      if (!found)
        throw new Error(
          `Build ${options.buildId} not found for ${core} ${version}`,
        );
      build = found;
    } else {
    ...
    // If file exists and we're not forcing a download, try to verify it
    if (fileExists && !options.forceDownload) {
      console.log(`File already exists at ${filePath}`);

      // If hash is available, verify it
      if (artifact.downloadType === "binary" && artifact.hash) {
        console.log(`Verifying existing file hash (${artifact.hashType})...`);
        const existingHash = await this.fs.calculateHash(
          filePath,
          artifact.hashType || "sha256",
        );

        if (existingHash === artifact.hash) {
          console.log("Existing file hash matches! Skipping download.");
          ...
          console.log(
            `Hash mismatch! Expected ${artifact.hash}, got ${existingHash}. Re-downloading...`,
          );
        }
      } else {
        console.log(
          "No hash available for verification. Assuming file is valid.",
        );
    ...
    if (artifact.downloadType === "binary" && artifact.hash) {
      ...
      const hex = await this.fs.calculateHash(
        filePath,
        artifact.hashType || "sha256",
      );
      ...
      console.log(
        'Download type is "path". Skipping hash verification of the path file.',
      );
    } else {
  ...
  async verifyFile(
    filePath: string,
    expectedHash?: string,
    hashType: "sha1" | "sha256" | "md5" = "sha256",
  ): Promise<boolean> {
    const fileExists = await this.fs.exists(filePath);
    if (!fileExists) {
      return false;
    ...
    if (expectedHash) {
      console.log(`Verifying file hash (${hashType})...`);
      const actualHash = await this.fs.calculateHash(filePath, hashType);
  ...
  async getFileInfo(
    ...
    const size = await this.fs.getFileSize(filePath);
    const hash = await this.fs.calculateHash(filePath, hashType);
